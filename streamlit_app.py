...
st.title("💬 지구촌 수도 챗봇")
st.write(
    "지구촌 수도 챗봇은 사용자가 어떤 나라의 수도를 물어보면 수도와 간단한 설명을 제공하는 목적 특화형 챗봇입니다. \n"
...
if not openai_api_key:
    ...
    if "system_prompt" not in st.session_state:
        st.session_state.system_prompt = (
            "당신은 지구촌 수도 챗봇입니다. 사용자가 어떤 나라의 수도를 물어보면 수도를 한 문장으로 알려주고, \n"
            "그 나라에 대한 간단한 설명(위치, 대표 특징, 간단한 역사/인구 정보 등)을 2~3문장으로 제공하세요. \n"
            "응답은 한국어로 작성해주세요. 질문이 국가의 수도와 관련 없을 경우에는 '이 챗봇은 국가의 수도를 알려주는 챗봇입니다. 수도를 물어보세요.'라고 안내하세요."
        )
    ...
    # Input prompt field
    if prompt := st.chat_input("예: '대한민국의 수도는 어디인가요?' — 나라 이름을 물어보세요." ):
